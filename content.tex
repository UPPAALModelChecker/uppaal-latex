...
The {\ttfamily uppaal.sty} package has been developed over many years by publishing papers related to \uppaal.
...
\subsection{Example}
Listing~\ref{lst:example1} shows the simplest \LaTeX{} code to typeset \uppaal code enclosed in {\ttfamily uppaalcode} environment:
...
\begin{description}
\item[\href{https://ctan.org/pkg/listings}{listings}] -- to produce an actual code listing.
...
\item[\href{https://ctan.org/pkg/bera}{bera}] -- \href{https://tug.org/FontCatalogue/beramono/}{Bitstream Vera Mono} (beramono) fonts, the directory is usually called {\ttfamily bera} and the font files are prefixed with {\ttfamily fvm}.
\end{description}
...
For example, \href{https://www.debian.org/distrib/packages}{Debian GNU/Linux} puts them into \href{https://packages.debian.org/sid/texlive-latex-recommended}{\ttfamily texlive-latex-recommended}, \href{https://packages.debian.org/sid/texlive-latex-recommended-doc}{\ttfamily texlive-latex-recommended-doc} and \href{https://packages.debian.org/sid/texlive-fonts-extra}{\ttfamily texlive-fonts-extra} packages.
...
The {\ttfamily uppaal.sty} defines \uppaal language styles for {\ttfamily listings.sty} package, thus all the hard lifting is done by {\ttfamily listings.sty} and the details can be found in \href{https://ctan.org/pkg/listings}{{\bfseries listings} documentation}.
\begin{description}
...
\item[frameround]
\end{description}
There are are many many more options, please see the documentation for \href{https://ctan.org/pkg/listings}{\ttfamily listings.sty} package.
It also plays nicely with \href{https://ctan.org/pkg/tcolorbox}{\ttfamily tcolorbox} package for fancy listings in \href{https://ctan.org/pkg/beamer}{\ttfamily beamer} presentations.
...
Tables~\ref{tab:commands} and~\ref{tab:tools} show a list of available commands.
...
\begin{tabular}{@{}l@{ }l@{ }l@{}}
  ...
  \midrule
  \multicolumn{3}{@{}l}{Maths macros:} \\
  Possibly & \cmdtt{exists\textbackslash uppD\textbackslash,p} & $\exists\uppD\,p$ \\
  Always Globally & \cmdtt{forall\textbackslash uppB\textbackslash,p} & $\forall\uppB\,p$ \\
  Leads-to & {\ttfamily p \textbackslash uppLeadsTo q} & $p \uppLeadsTo q$ \\
  \bottomrule
\end{tabular}
\end{table}

\begin{table}
  \caption{Small-caps for tool names: \uppaal\cora, \uppaal\tiga, \uppaal\tron, \uppaal\smc, \uppaal\stratego.}\label{tab:tools}
  \begin{tabular}{@{}r|llllll@{}}
    \toprule
    {\bfseries Macro} & {\bfseries Normal} & {\bfseries Lower} & {\bfseries Upper} & {\bfseries Title} & {\bfseries Bold} & {\bfseries Emphasize} \\
    \midrule
    Uppaal & Uppaal & \MakeLowercase{Uppaal} & \MakeUppercase{Uppaal} & \MakeTitlecase{Uppaal} & {\bfseries Uppaal} & {\em Uppaal} \\
    \textbackslash uppaal & \uppaal & \MakeLowercase{\uppaal} & \MakeUppercase{\uppaal} & \MakeTitlecase{\uppaal} & {\bf\uppaal} & {\em\uppaal} \\
    \textbackslash cora & \cora & \MakeLowercase{\cora} & \MakeUppercase{\cora} & \MakeTitlecase{\cora} & {\bf\cora} & {\em\cora} \\
    \textbackslash coshy & \cora & \MakeLowercase{\coshy} & \MakeUppercase{\coshy} & \MakeTitlecase{\coshy} & {\bf\coshy} & {\em\coshy} \\
    \textbackslash tiga & \tiga & \MakeLowercase{\tiga} & \MakeUppercase{\tiga} & \MakeTitlecase{\tiga} & {\bf\tiga} & {\em\tiga} \\
    \textbackslash tron & \tron & \MakeLowercase{\tron} & \MakeUppercase{\tron} & \MakeTitlecase{\tron} & {\bf\tron} & {\em\tron} \\
    \textbackslash smc & \smc & \MakeLowercase{\smc} & \MakeUppercase{\smc} & \MakeTitlecase{\smc} & {\bf\smc} & {\em\smc} \\
    \textbackslash stratego & \stratego & \MakeLowercase{\stratego} & \MakeUppercase{\stratego} & \MakeTitlecase{\stratego} & {\bf\stratego} & {\em\stratego} \\
    \bottomrule
  \end{tabular}
\end{table}
